...
| `js/core/dom.js` | `window.QSDom`: shared `el()` element factory + `svgIcon()` + `clear()` + `setSelectPlaceholder()` (resets a `<select>` to a single placeholder option) — the createElement/textContent idiom for dynamic DOM. Loaded in `<head>` so page scripts can use it at parse time. |
...
otherwise learn half a credential. (Hiding it is not what keeps such a holder from
locking the owner out: failed password changes and deletions on this page count
against the owner's login throttle, username or not.) After the login page that
shows a newly assigned username until a sign-in in that browser — for 24 hours at
most — nothing in the panel displays it.
