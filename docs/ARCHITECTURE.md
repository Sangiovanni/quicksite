...
| **Management** | `secure/management/` | API client (admin panel, scripts) | The 125 commands that read or mutate project data. Single entry point: `public/management/index.php`. Token + role enforced. AI calls bypass this layer entirely (browser-direct). |
...
The full list of 125 commands is registered in `secure/management/routes.php`. See [COMMAND_API.md](COMMAND_API.md) for the catalogue and a per-command reference (also obtainable at runtime via `GET /management/help`).
...
The same pattern — parse → validate → mutate files → `ApiResponse` — is used by all 125 commands.
