...
Accounts are **self-created only**. `register`, and the `/admin/register` page beside it, creates one when `auth.php` allows self-registration — the default is `false`, so a fresh install accepts no registrations and accounts exist from setup only. Registration is enumeration-safe because nobody chooses a username: the server assigns one, so there is no taken name to reveal, and hands it only to the person registering — in the `register` response, or on that browser's login page until a sign-in in that browser, for 24 hours at most.
...
| Inline JS injection | `on*` attributes only accept the `{{call:fn:args}}` syntax (see §9), transformed to allowlisted `QS.*()` calls (`CallTransformer`); raw JS is rejected. The tag policy is also enforced by the commands that take a tag or a whole structure from the request (`addNode`, `editNode`, `editStructure`, `addComplexElement`, `createSnippet`) and by `importProject` over every structure in an archive, and the attribute-name, URL-scheme and `on*` policies by every command that writes a structure file, so unsafe values are refused on write, not just dropped at render. The write-side check is a second layer, never the only one: projects can already hold structures written before a rule existed, and only the render/compile gate protects a page built from those. |
