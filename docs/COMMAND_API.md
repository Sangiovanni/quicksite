...
**These commands develop a project.** Anything about the installation itself, the account, or the admin panel's own state is deliberately *not* here: the panel serves those from its own endpoints (`/admin/api`, `/admin/state`), so a script driving this API gets a surface that is about websites and nothing else. Two examples you may be looking for: the update check is `GET /admin/api/update-check`, and switching which project the panel edits is `POST /admin/state/selected-project`.
...
The 170 commands group into the categories below. Use `GET /management/help` for the full per-command spec.
...
| **Roles** | `listRoles`, `getMyPermissions`, `createRole`, `editRole`, `deleteRole` — `listRoles` and `getMyPermissions` read the fixed roles and your own effective permissions. **`createRole`, `editRole` and `deleteRole` are routed but usable by nobody**: roles are a fixed set, not customisable, so the three sit in a category no role grants. The dispatcher accepts their names and the permission check then refuses every call with `403 auth.forbidden` — the project owner's included. They are listed here because the names resolve, not because they do anything. |
...
- The whitelist of valid commands is `secure/management/routes.php` (170 entries — this file is the single source of truth for which commands exist).
