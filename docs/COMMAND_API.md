...
A **structure file** — any `.json` file under `templates/model/json/` or `snippets/` — is refused the way a limit breach is, not the way an entry is. One that cannot be read or parsed, fails the content check, or holds an unsafe attribute, a blocked tag or an invalid component reference refuses the whole archive before any project is created, because skipping it would import a site with part of its structure missing — a page whose route still exists, say. The answer is `400 validation.unsafe_param`, with `errors[0].file` naming the entry and `errors[0].reason` the check it failed.
