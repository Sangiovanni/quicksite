...
| **Builds** | `build`, `listBuilds`, `getBuild`, `deleteBuild`, `cleanBuilds`, `deployBuild`, `downloadBuild` — compile a project to a static deliverable under its own `public/build/<name>/`; list / deploy (to the install root, or a root listed in `secure/management/config/deploy-roots.php`) / clean. |
