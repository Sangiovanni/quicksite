...
returns full documentation for **all 167 commands** — parameters, examples, validation rules, and error codes. For a specific command:
...
The 167 commands group into the categories below. Use `GET /management/help` for the full per-command spec.
...
| **Project members** | `listMembers`, `inviteMember`, `cancelInvitation`, `changeMemberRole`, `removeMember`, `transferOwnership` — the project's roster, on a consent model: an admin/owner *invites* an existing account (by `user_id`, discovered via `findUser`) and membership materializes only when the invitee accepts. Rank rules throughout: you can only offer, change to, cancel, or remove roles of strictly lower rank than your own; the owner's role is immutable except via `transferOwnership` (owner-only, member-only target, `confirm: true`, departing owner keeps `old_owner_role` — default `admin`). Members are referenced as `{user_id, name}` — the public display name and the opaque id; the private login username never appears. All six are project-scoped on the URL marker (`/management/p/<projectId>/…`) and ignore any project named in the body. |
| **My memberships** | `findUser`, `listMyInvitations`, `acceptInvitation`, `declineInvitation`, `leaveProject`, `dismissProjectNotice` — the caller's own membership surface (global, any authenticated user; self-service is deliberately not project-marker-scoped since an invitee is not yet a member). `findUser` = exact public-name lookup returning `{user_id, name}` matches (names are not unique — the id disambiguates). `listMyInvitations` = the inbox: pending invitations (offer detail read from the project's authoritative members.json) plus terminal notices (`removed` / `deleted` / `refused`) awaiting `dismissProjectNotice`. `acceptInvitation` re-validates the inviter's authority at accept time — a demoted or removed inviter's offer is void and never grants. The owner cannot `leaveProject` (transfer ownership first). |
...
- The whitelist of valid commands is `secure/management/routes.php` (167 entries — this file is the single source of truth for which commands exist).
