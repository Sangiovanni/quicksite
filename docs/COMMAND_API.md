...
Two kinds of entry are refused the way a limit breach is, not the way an entry is. An entry in the project folder whose **name** is not a clean relative path — absolute, or with an empty segment, a `.` or `..` segment, or a segment ending in a dot or a space — refuses the whole archive, because another spelling of a path can land on a real file without being recognised as it. And a **file the site reads** — `config.json` and `routes.json` at the project root, any `.json` file under `config/`, `translate/` or `data/`, and every **structure file** (any `.json` file under `templates/model/json/` or `snippets/`) — refuses the whole archive when it cannot be read, does not parse as a JSON array or object, or fails the content check, a structure file also when it holds an unsafe attribute, a blocked tag or an invalid component reference, and `config.json` also when it holds a setting the command that writes it would refuse. Skipping one would import a site with part of it missing — a page whose route still exists, a language showing raw keys, settings and routes silently replaced by the defaults. A file at a hidden path is neither: the import never writes a hidden path. Both refusals come before any project is created. The answer is `400 validation.unsafe_param`, with `errors[0].file` naming the entry and `errors[0].reason` the check it failed.
...
A refused entry is skipped and reported with its reason under `security.skipped_disallowed`; the rest of the archive still imports — unless it is a file the site reads, which refuses the whole archive (see above).
...
| Delete your account | `POST /admin/self/delete` — `{current_password, username, confirm: true}` |
...
- **Both credential operations require the current password**, on the same
  ...
  an account on its own. Deletion additionally needs your username and `confirm: true` and is
