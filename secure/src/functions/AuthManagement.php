...
/**
 * The bcrypt cost of every password hash this install writes: 12, PHP 8.4's own
 * default. A convention, not a setting.
 *
 * WHY A NAMED COST. The login gate answers an unknown username by verifying
 * against a dummy hash, so that a real username and an unknown one take the same
 * time. That holds only while the dummy and the stored hashes share one cost,
 * and PASSWORD_DEFAULT promises none: PHP 8.4 raised its bcrypt cost from 10 to
 * 12. At 12 against 10, a real username is refused about 130 ms slower — one
 * request tells them apart. So the cost is named here, every hash is made at it
 * (qs_password_hash), the dummy is built from it (qs_password_dummy_hash), and a
 * hash made at any other cost converges on it at its account's next sign-in
 * (qs_password_needs_rehash).
 */
const QS_PASSWORD_BCRYPT_COST = 12;

/** Hash a password at the install's algorithm and cost. */
function qs_password_hash(string $password): string {
    return password_hash($password, PASSWORD_BCRYPT, ['cost' => QS_PASSWORD_BCRYPT_COST]);
}

/**
 * Was this stored hash made at anything but the install's algorithm and cost —
 * an older cost, another algorithm, a hash an operator generated by hand?
 */
function qs_password_needs_rehash(string $hash): bool {
    return password_needs_rehash($hash, PASSWORD_BCRYPT, ['cost' => QS_PASSWORD_BCRYPT_COST]);
}

/**
 * A hash no password matches, at the install's algorithm and cost — what the
 * login gate verifies against when there is no real hash to check.
 *
 * Built from the constant rather than written out, so it cannot drift from the
 * cost real hashes are made at. Verifying a bcrypt string costs 2^cost rounds
 * whatever its salt and digest say, and those two here are fixed text in
 * bcrypt's own alphabet: a 22-character salt, a 31-character digest.
 */
function qs_password_dummy_hash(): string {
    return sprintf('$2y$%02d$', QS_PASSWORD_BCRYPT_COST)
        . 'QuickSiteTimingDummy..'
        . 'NoPasswordEverVerifiesAgainstIt';
}

/**
 * Re-hash one account's password at the install's cost — called by the login
 * gate once a sign-in has proved the password, for a stored hash made at any
 * other cost or algorithm, so that account converges instead of staying
 * distinguishable by how long its refusals take.
 *
 * Written only while the stored hash is still the one just verified: a password
 * change that lands in between wins. Never fatal — if the new hash cannot be
 * made or written, the old one stays and the next sign-in tries again. Nothing
 * else on the record moves; the session generation in particular is untouched,
 * because nothing about the password changed.
 */
function qs_password_rehash(string $userId, string $verifiedHash, string $password): void {
    try {
        $newHash = qs_password_hash($password);
    } catch (Throwable $e) {
        return;
    }
    qs_users_mutate(function (array &$cfg) use ($userId, $verifiedHash, $newHash) {
        if (($cfg['users'][$userId]['password_hash'] ?? null) !== $verifiedHash) {
            return false;
        }
 ...
 * a dummy hash at the same algorithm and cost when the user has no usable
 * password (qs_password_dummy_hash).
 *
 * The attempt is admitted and counted by the throttle before anything is
 * verified (qs_login_throttle_admit), so a refusal needs nothing more recorded;
...
function qs_auth_attempt_login(string $username, string $password): array {
    ...
    $wait = qs_login_throttle_admit($username);
    if ($wait > 0) {
    ...
    if (is_string($hash) && $hash !== '') {
        ...
        password_verify($password, qs_password_dummy_hash());
        $verified = false;
    }
    ...
    if (qs_password_needs_rehash($hash)) {
        qs_password_rehash((string)$user['id'], $hash, $password);
    }
 ...
 *
 * The bcrypt hash (qs_password_hash) is computed BEFORE the lock: at
 * QS_PASSWORD_BCRYPT_COST it takes a fifth of a second or so, and the write lock
 * must not be held that long.
 *
...
function qs_user_create(string $name, string $username, ?string $password, int $maxUsers = 0, ?string $setupToken = null): array {
    ...
    $hash = ($password !== null && $password !== '') ? qs_password_hash($password) : null;
...
/**
 * THE FIRST-RUN GATE — the bootstrap sibling of qs_auth_attempt_login /
 ...
 *
 * Order matters. An incomplete form is answered first and costs no budget — it
 * guessed nothing. The token is then checked AFTER the throttle has admitted and
 * counted the attempt, so guessing costs budget, and BEFORE the password policy,
 * so someone guessing tokens learns nothing about the install's rules. A token
 * that verifies clears the count: the caller has proved what the throttle
 * guards, so nothing refused after that point — a username that does not fit, a
 * password too short, a creation that lost a race — was a guess.
 *
...
function qs_auth_attempt_setup(string $name, string $username, string $password, string $token): array {
    ...
    if (!qs_setup_token_verify($token)) {
        return ['ok' => false, 'error' => 'invalid_token']; // counted when admitted
    }
 ...
 * a cookie of its own.
 *
 * Flood control counts the REGISTRATION, not the draws, and is admitted before
 * anything is drawn (qs_registration_throttle_admit): one attempt against the
 * caller's IP whatever happens next, and a place in the install-wide hourly cap
 * that only a real creation keeps — every other ending hands it back.
 *
 ...
 * 'setup_required' (while self-registration is off, 'registration_disabled'
 * answers first) — the first account is created only through the first-run
...
function qs_auth_attempt_register(string $name, string $password): array {
    ...
    $hour = null;
    $wait = qs_registration_throttle_admit($cfg, $hour); // once per registration, however many draws
    if ($wait > 0) {
        return ['ok' => false, 'error' => 'throttled', 'retry_after' => $wait];
    }

    // The hourly place just reserved is kept only by a real creation; every
    // other way out of this block hands it back, an exception included.
    $created = null;
    try {
        ...
        if (!($created['ok'] ?? false)) {
            qs_registration_throttle_release($hour);
        }
    }
}
