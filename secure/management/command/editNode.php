...
function __command_editNode(array $params = [], array $urlParams = []): ApiResponse {
    ...
    // the edited node's final params included, which is where a second
    // spelling of an existing name would land.
    ...
    try {
        ...
        $renderedHtml = $renderer->renderNodeAtPath($structure, $nodeId, $structureName);
    } catch (Exception $e) {
        // Non-fatal - we can still return success without rendered HTML
        error_log("editNode: Failed to render node HTML: " . $e->getMessage());
    }
