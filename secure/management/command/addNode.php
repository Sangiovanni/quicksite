...
function __command_addNode(array $params = [], array $urlParams = []): ApiResponse {
    ...
    if ($isRootTarget) {
        // Force position to 'inside' when targeting root
        $position = 'inside';
    ...
    // Alt is now a plain string param (not a translation key).
    // If no alt provided for tags that require it, set an empty placeholder.
    if (in_array($tag, TagRegistry::TAGS_WITH_ALT, true) && !isset($nodeParams['alt'])) {
        $nodeParams['alt'] = '';
    }
    ...
    // four are exempt.
    if ($tag === 'input') {
        $inputType = $nodeParams['type'] ?? '';
        $exemptTypes = ['submit', 'reset', 'button', 'hidden'];
        if (!in_array($inputType, $exemptTypes, true)
            && (!isset($nodeParams['name']) || trim((string)$nodeParams['name']) === '')) {
            return ApiResponse::create(400, 'validation.required')
                ->withMessage("Input of type '{$inputType}' requires a 'name' attribute to be submitted with the form.")
                ->withErrors([[
                    'field' => 'params.name',
                    'reason' => 'required_for_input_type',
                    'tag' => 'input',
                    'type' => $inputType,
                ]]);
        }
    ...
    if ($isRootTarget) {
        // Root target = the structure itself
    ...
    // (No alt-key block here. `alt` became a plain string param above — nothing
    // in this command generates a translation key for it any more, so reading
    // $autoGeneratedAltKey emitted an "Undefined variable" warning on EVERY
    // node insertion and the two response fields it guarded were never set.
    // editNode still generates one and keeps its own block.)
