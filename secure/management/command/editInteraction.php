...
function __command_editInteraction(array $params = [], array $urlParams = []): ApiResponse {
    ...
    // Validate newEvent if provided
    if ($newEvent !== null && $newEvent !== $event) {
        if (!in_array($newEvent, $allEvents, true)) {
            return ApiResponse::create(400, 'validation.invalid_value')
                ->withMessage("Invalid newEvent: {$newEvent}")
                ->withErrors([['field' => 'newEvent', 'value' => $newEvent]]);
        }
    }
    ...
    // Check if event is being changed
    if ($newEvent !== null && $newEvent !== $event) {
        // Remove interaction from old event
        array_splice($existingInteractions, $index, 1);
        
        if (count($existingInteractions) === 0) {
            // No more interactions on old event, remove the param
            unset($node['params'][$event]);
        } else {
            // Rebuild old event value without the removed interaction
            $node['params'][$event] = implode(' ', array_map(fn($i) => $i['raw'], $existingInteractions));
        }
        
        // Add interaction to new event
        $existingNewValue = $node['params'][$newEvent] ?? '';
        if (!empty($existingNewValue)) {
            $node['params'][$newEvent] = $existingNewValue . ' ' . $newCallSyntax;
        } else {
            $node['params'][$newEvent] = $newCallSyntax;
        }
    } else {
        // Same event - replace the interaction at index
    ...
    $responseData = [
        'event' => ($newEvent !== null && $newEvent !== $event) ? $newEvent : $event,
        'index' => $index,
