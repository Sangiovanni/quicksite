...
window.PreviewConfig = {
    ...
    i18n: {
        ...
        noResponseSchemaHint: <?= json_encode(__admin('preview.noResponseSchemaHint', 'Response Mapping needs one. Add it on /admin/apis — open this endpoint and paste a JSON schema describing the response. There is no inference from a test call; the field list below comes from the schema alone.')) ?>,
        modeNeedsArray: <?= json_encode(__admin('preview.modeNeedsArray', 'List / Component / Count need a field the response schema declares as an array. This one is declared')) ?>,
        modeNeedsArrayUntyped: <?= json_encode(__admin('preview.modeNeedsArrayUntyped', 'List / Component / Count need a field the response schema declares as an array. This one declares no type.')) ?>,
        countKeysUnresolved: <?= json_encode(__admin('preview.countKeysUnresolved', 'Saved. Some count sentences have no text yet')) ?>,
