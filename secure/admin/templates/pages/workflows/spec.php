...
<div class="ai-spec" data-spec-id="<?= htmlspecialchars($specId) ?>" data-is-create-spec="<?= $hasCreateTag ? 'true' : 'false' ?>" data-has-no-params="<?= empty($parameters) ? 'true' : 'false' ?>" data-is-manual="<?= $isManualWorkflow ? 'true' : 'false' ?>" data-is-paste-only="<?= $isPasteOnly ? 'true' : 'false' ?>">
    ...
    <header class="ai-spec__header">
        ...
        <?php if (($spec['_source'] ?? 'core') === 'custom'): ?>
        <div class="ai-spec__actions">
            <a href="<?= $router->url('workflows', 'edit/' . $specId) ?>" class="admin-btn admin-btn--primary">
            ...
            <button type="button" id="delete-workflow-btn" class="admin-btn admin-btn--danger" data-id="<?= htmlspecialchars($specId) ?>">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <?= __admin('workflows.spec.delete', 'Delete') ?>
            </button>
        </div>
        <?php endif; ?>
    </header>
