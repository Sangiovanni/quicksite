...
﻿(function() {
    ...
    function renderTransformEditor() {
        ...
        transformFunctions.forEach((func, index) => {
            ...
            row.innerHTML = `
                <div class="transform-editor__drag-handle" title="${PreviewConfig.i18n.dragToReorder}">â‹®â‹®</div>
    ...
    async function loadTransitionSelectorStates(selector) {
        ...
        try {
            ...
            showToast(PreviewConfig.i18n.loadTransitionFailed, 'error');
        }
    }
    ...
    function renderAnimationSection(target) {
        ...
        if (target === 'trigger' && transitionTriggerAnimHint) {
            ...
            transitionTriggerAnimHint.textContent = `(${PreviewConfig.i18n.playsOn} ${pseudo})`;
        }
    }
    ...
    function renderTransitionBaseProperties() {
        ...
        if (props.length === 0) {
            transitionBasePropsEl.innerHTML = `
                <div class="transition-editor__empty">${PreviewConfig.i18n.noBaseStyles}</div>
            `;
            return;
        }
    ...
    function renderTransitionHoverProperties() {
        ...
        if (props.length === 0) {
            transitionHoverPropsEl.innerHTML = `
                <div class="transition-editor__empty">${PreviewConfig.i18n.noTriggerStyles}</div>
            `;
            return;
        }
    ...
    async function deleteTransitionProperty(target, property) {
        ...
        try {
            ...
            if (result.status === 200) {
                showToast(PreviewConfig.i18n.propertyDeleted, 'success');
            ...
            showToast(PreviewConfig.i18n.deletePropertyFailed, 'error');
        }
    }
    ...
    async function addTransitionProperty(target, property, value) {
        ...
        try {
            ...
            if (result.status === 200) {
                showToast(PreviewConfig.i18n.propertyAdded, 'success');
                // Reload the properties
                await loadTransitionSelectorStates(transitionEditorSelector);
                // Refresh iframe
            ...
            showToast(PreviewConfig.i18n.addPropertyFailed, 'error');
        }
    }
        ...
        if (!keyframesData || keyframesData.length === 0) {
            ...
            .then(data => {
                if (data.status === 200 && data.data) {
                    ...
                    showToast(PreviewConfig.i18n.noKeyframesFound, 'warning');
                }
            })
            .catch(() => {
                showToast(PreviewConfig.i18n.errorLoadingKeyframes, 'error');
            });
        } else {
            showAnimationPickerDropdown(target);
        }
    ...
    async function addAnimationToSelector(target, keyframeName) {
        ...
        try {
            ...
            const result = await response.json();
            if (result.status === 200) {
                showToast(PreviewConfig.i18n.animationAdded, 'success');
                ...
                showToast(result.message || PreviewConfig.i18n.errorAddingAnimation, 'error');
            }
        } catch (error) {
            console.error('Failed to add animation:', error);
            showToast(PreviewConfig.i18n.errorAddingAnimation, 'error');
        }
    }
    ...
    async function removeAnimation(target) {
        ...
        try {
            ...
            const result = await response.json();
            if (result.status === 200) {
                showToast(PreviewConfig.i18n.animationRemoved, 'success');
                ...
                showToast(result.message || PreviewConfig.i18n.errorRemovingAnimation, 'error');
            }
        } catch (error) {
            console.error('Failed to remove animation:', error);
            showToast(PreviewConfig.i18n.errorRemovingAnimation, 'error');
        }
    }
    ...
    function previewAnimation(target) {
        ...
        if (!animation || !animation.name) {
            showToast(PreviewConfig.i18n.noAnimationToPreview, 'warning');
            return;
        }
        ...
        if (!keyframe) {
            showToast(PreviewConfig.i18n.keyframeNotFound, 'error');
            return;
        }
        ...
        if (typeof openAnimationPreviewModal === 'function') {
            ...
            showToast(PreviewConfig.i18n.previewNotAvailable, 'warning');
        }
    }
    ...
    function updateSpecificPropertiesCheckboxes() {
        ...
        if (propsToShow.length === 0) {
            transitionSpecificProps.innerHTML = `
                <div class="transition-editor__empty">${PreviewConfig.i18n.noPropertiesToTransition}</div>
            `;
            return;
        }
    ...
    function updateTriggerStateLabels() {
        ...
        if (transitionTriggerLabel) {
            transitionTriggerLabel.textContent = `${pseudo} ${PreviewConfig.i18n.state}`;
        }
        
        // Update the "Add State Style" button text
        if (transitionAddHoverText) {
            transitionAddHoverText.textContent = `${PreviewConfig.i18n.addStyle} ${pseudo} ${PreviewConfig.i18n.style}`;
        }
    }
    ...
    async function saveTransition() {
        ...
        try {
            ...
            if (result.status === 200) {
                showToast(PreviewConfig.i18n.transitionSaved, 'success');
            ...
            showToast(PreviewConfig.i18n.saveTransitionFailed, 'error');
        }
    }
    ...
    function previewTransitionHover() {
        ...
        try {
            ...
            if (elements.length === 0) {
                showToast(PreviewConfig.i18n.noElementsFound, 'warning');
                return;
            }
            ...
            showToast(PreviewConfig.i18n.previewingHover, 'info');
    ...
    async function loadThemeVariables() {
        ...
        try {
            ...
            themeLoading.innerHTML = `
                ...
                <span style="color: #ef4444;">${PreviewConfig.i18n.themeLoadError}</span>
            `;
        }
    }
    ...
    function renderColorInputs(container, variables) {
        ...
        if (variables.length === 0) {
            container.innerHTML = `<p class="preview-theme-empty">${PreviewConfig.i18n.noColorVariables}</p>`;
            return;
        }
    ...
    function renderFontInputs(container, variables) {
        ...
        if (variables.length === 0) {
            container.innerHTML = `<p class="preview-theme-empty">${PreviewConfig.i18n.noFontVariables}</p>`;
            return;
        }
    ...
    function renderSpacingInputs(container, variables) {
        ...
        if (variables.length === 0) {
            container.innerHTML = `<p class="preview-theme-empty">${PreviewConfig.i18n.noSpacingVariables}</p>`;
            return;
        }
    ...
    async function saveThemeVariables() {
        ...
        try {
            ...
            if (data.status === 200 || data.status === 201) {
                ...
                // Show success toast
                showToast(PreviewConfig.i18n.themeSaved, 'success');
            ...
            showToast(PreviewConfig.i18n.themeSaveError, 'error');
        } finally {
            // Restore button
    ...
    function resetThemeVariables() {
        ...
        showToast(PreviewConfig.i18n.themeReset, 'info');
    }
    ...
    function populateAnimatedSelectorsList() {
        ...
        if (transCount > 0) {
            ...
            transitionsList.innerHTML = '<div class="preview-animated-selector--empty">' + PreviewConfig.i18n.noTransitions + '</div>';
        }
        ...
        if (animCount > 0) {
            ...
            animationsList.innerHTML = '<div class="preview-animated-selector--empty">' + PreviewConfig.i18n.noAnimationSelectors + '</div>';
        }
        ...
        if (triggersList) {
            if (triggerCount > 0) {
                ...
                triggersList.innerHTML = '<div class="preview-animated-selector--empty">' + PreviewConfig.i18n.noTriggersWithoutTransition + '</div>';
            }
        }
        ...
        return `
            <div class="preview-animated-selector-group preview-animated-selector-group--trigger">
                <div class="preview-animated-selector-group__header preview-animated-selector-group__header--clickable" data-selector="${escapeHtml(item.selector || item.baseSelector)}">
                    ...
                    <span class="preview-animated-selector__state-count preview-animated-selector__state-count--muted">${item.states.length} ${PreviewConfig.i18n.states}</span>
                    <svg class="preview-animated-selector__arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="12" height="12">
                        <path d="M5 12h14"/><path d="M12 5l7 7-7 7"/>
                    </svg>
                </div>
    ...
    function renderKeyframeTimeline() {
        ...
        keyframeFrames.forEach((frame, index) => {
            ...
            marker.title = frame.percent + '% - ${PreviewConfig.i18n.dragToMoveFrame}';
    ...
    async function loadStyleSelectors() {
        ...
        if (success) {
            ...
            selectorsLoading.innerHTML = `
                ...
                <span style="color: #ef4444;">${PreviewConfig.i18n.selectorsLoadError}</span>
            `;
        }
    }
    ...
    function createSelectorChip(selector, type, mediaQuery = null) {
        ...
        // Hover â†’ highlight elements in iframe
    ...
    function showVarReplacementWarning(property, newValue) {
        showToast(
            PreviewConfig.i18n.varReplacementWarning,
            'warning',
            5000
        );
    }
    ...
    async function handleElementMoved(data) {
        ...
        if (!sourceEl || !targetEl) {
            ...
            showToast(PreviewConfig.i18n.error + ': Elements not found', 'error');
            return;
        }
    ...
    function displayInteractions(data) {
        ...
        if (!data.interactions || data.interactions.length === 0) {
            listEl.innerHTML = '<p class="preview-contextual-js-empty">${PreviewConfig.i18n.noInteractions}</p>';
            return;
        }
    ...
    async function applyStyles() {
        ...
        if (!selector) {
            showToast(PreviewConfig.i18n.error + ': Cannot determine selector', 'error');
            return;
        }
    ...
    function createSearchablePicker(inputType, arg, index) {
        ...
        function renderDropdown(filter = '') {
            ...
            filtered.forEach(item => {
                ...
                const typeIcon = item.type === 'special' ? 'âš¡' : 
                                 item.type === 'id' ? '#' : 
                                 item.type === 'class' ? '.' : 
                                 item.type === 'dom' ? 'â—‡' :
                                 item.type === 'common' ? 'â˜…' : 
    ...
    if (jsFormFunction) {
        jsFormFunction.addEventListener('change', () => {
            ...
            if (args.length === 0) {
                jsFormParams.innerHTML = '<p class="preview-contextual-js-form-hint">${PreviewConfig.i18n.noParams}</p>';
            } else {
    ...
    function addCustomParamRow(key = '', value = '') {
        ...
        row.innerHTML = `
            <input type="text" 
                   ...
                   placeholder="${PreviewConfig.i18n.paramName}"
                   value="${key}">
            <input type="text" 
                   class="admin-input preview-add-node-modal__param-value" 
                   placeholder="${PreviewConfig.i18n.paramValue}"
                   value="${value}">
            <button type="button" class="preview-add-node-modal__remove-param" data-remove="${customParamsCount}" title="${PreviewConfig.i18n.remove}">
    ...
    async function loadComponentsList() {
        ...
        addNodeComponentSelect.innerHTML = '<option value="">${PreviewConfig.i18n.loading}...</option>';
        ...
        try {
            ...
            if (result.ok && result.data?.data?.components) {
                ...
                addNodeComponentSelect.innerHTML = '<option value="">${PreviewConfig.i18n.error}</option>';
            }
        } catch (error) {
    ...
    function populateComponentSelect(components) {
        addNodeComponentSelect.innerHTML = '<option value="">${PreviewConfig.i18n.selectComponentPlaceholder}</option>';
        
        if (components.length === 0) {
            addNodeComponentSelect.innerHTML = '<option value="">${PreviewConfig.i18n.noComponents}</option>';
            return;
        }
            ...
            if (varInfo.type === 'textKey') {
                ...
                row.innerHTML = `
                    ...
                    <div class="preview-add-node-modal__param-readonly" style="display: flex; align-items: center; gap: 8px; padding: 8px 12px; background: var(--admin-bg-secondary, #f5f5f5); border-radius: 4px; flex: 1;">
                        ...
                        <small style="color: var(--admin-text-muted, #6b7280); font-style: italic;">${PreviewConfig.i18n.autoGenerated}</small>
                    </div>
                `;
            } else {
                ...
                row.innerHTML = `
                    ...
                    <input type="text" 
                           ...
                           placeholder="${PreviewConfig.i18n.enterValue}">
                `;
            }
    ...
    if (addNodeConfirm) {
        addNodeConfirm.addEventListener('click', async function() {
            ...
            if (currentNodeType === 'component') {
                ...
                const componentName = addNodeComponentSelect?.value;
                if (!componentName) {
                    showToast(PreviewConfig.i18n.selectComponent, 'warning');
                    return;
                }
                ...
                try {
                    ...
                    let successMsg = PreviewConfig.i18n.componentAdded + `: ${componentName}`;
                    if (data.nodeId) {
                ...
                for (const param of requiredParams) {
                    if (!params[param]) {
                        showToast(PreviewConfig.i18n.paramRequired + `: ${param}`, 'warning');
                        return;
                    }
                }
                ...
                try {
                    ...
                    const data = result.data?.data || {};
                    let successMsg = PreviewConfig.i18n.elementAdded;
                    if (data.newNodeId) {
    ...
    function createEditParamRow(key, value, isMandatory = false, isClass = false) {
        ...
        row.innerHTML = `
            ...
            ${canRemove ? `
            <button type="button" class="preview-add-node-modal__remove-param" title="${PreviewConfig.i18n.remove}">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width: 16px; height: 16px;">
                    <line x1="18" y1="6" x2="6" y2="18"/>
                    <line x1="6" y1="6" x2="18" y2="18"/>
                </svg>
    ...
    async function openEditNodeModal() {
        ...
        if (!element) {
            showToast(PreviewConfig.i18n.error + ': Element not found', 'error');
            return;
        }
    ...
    if (editNodeConfirm) {
        editNodeConfirm.addEventListener('click', async function() {
            ...
            try {
                ...
                const data = result.data?.data || {};
                let successMsg = PreviewConfig.i18n.elementUpdated;
        ...
        if (varInfo.type === 'param') {
            ...
            row.innerHTML = `
                ...
                <div class="preview-add-node-modal__var-readonly">
                    ...
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width: 14px; height: 14px; opacity: 0.5;" title="${PreviewConfig.i18n.editInTranslations}">
    ...
    async function openEditComponentModal() {
        if (!selectedStruct || !selectedNode || !selectedComponent) {
            showToast(PreviewConfig.i18n.selectComponentFirst, 'warning');
            return;
        }
    ...
    if (editComponentConfirm) {
        editComponentConfirm.addEventListener('click', async function() {
            ...
            try {
                ...
                const data = result.data?.data || {};
                showToast(PreviewConfig.i18n.componentUpdated, 'success');
