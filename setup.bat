...
echo } >> "%PS_SPACE_TEMP%"
echo. >> "%PS_SPACE_TEMP%"
echo # Regenerate nginx dynamic_routes.conf if it exists >> "%PS_SPACE_TEMP%"
echo $secureDir = '%SECURE_DIR%' -replace '/','\'  >> "%PS_SPACE_TEMP%"
echo $nginxConf = Join-Path $secureDir 'nginx\dynamic_routes.conf' >> "%PS_SPACE_TEMP%"
echo if (Test-Path $nginxConf) { >> "%PS_SPACE_TEMP%"
echo   $pfx = if ($newSpace) { "/$newSpace" } else { '' } >> "%PS_SPACE_TEMP%"
echo   $loc = if ($pfx) { "$pfx/" } else { '/' } >> "%PS_SPACE_TEMP%"
echo   $dt = Get-Date -Format 'yyyy-MM-dd HH:mm:ss' >> "%PS_SPACE_TEMP%"
echo   $nl = [Environment]::NewLine >> "%PS_SPACE_TEMP%"
echo   $cfg = "# ==========================================================" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# QuickSite - nginx dynamic routes configuration" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# ==========================================================" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# Auto-generated on $dt by QuickSite setup.bat" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# Do NOT edit manually - regenerated when public space changes." + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "#" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# Usage: Include this file in your nginx server {} block:" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "#   include /path/to/secure/nginx/dynamic_routes.conf;" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "#" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# Manual reload: nginx -t ^&^& nginx -s reload" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# ==========================================================" + $nl + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# Admin panel API (AJAX helper for dynamic form fields)" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "location $pfx/admin/api/ {" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "    try_files `$uri `$uri/ $pfx/admin/api/index.php`$is_args`$args;" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "}" + $nl + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# Management API (QuickSite command endpoint)" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "location $pfx/management/ {" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "    try_files `$uri `$uri/ $pfx/management/index.php`$is_args`$args;" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "}" + $nl + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# Admin panel" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "location $pfx/admin/ {" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "    try_files `$uri `$uri/ $pfx/admin/index.php`$is_args`$args;" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "}" + $nl + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "# Public site (catch-all for QuickSite routes)" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "location $loc {" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "    try_files `$uri `$uri/ $pfx/index.php`$is_args`$args;" + $nl >> "%PS_SPACE_TEMP%"
echo   $cfg += "}" + $nl >> "%PS_SPACE_TEMP%"
echo   [IO.File]::WriteAllText($nginxConf, $cfg, [System.Text.Encoding]::UTF8) >> "%PS_SPACE_TEMP%"
echo   Write-Host '  + Updated nginx dynamic_routes.conf' >> "%PS_SPACE_TEMP%"
echo } >> "%PS_SPACE_TEMP%"
echo. >> "%PS_SPACE_TEMP%"
