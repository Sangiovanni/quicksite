...
| `secure/src/classes/PageManagement.php` (hydration emit — `window.QS_RESOLVED` + `window.QS_RESOLVED_BY_INDEX`) | `ADMIN_PANEL.md` §9.7 (hydration handoff), `ARCHITECTURE.md` §9.4 |
...
| **Adding ANY new directory under `public/admin/`** (a JSON endpoint, an asset folder — anything) | ⚠ **RUN `NOTES/tests/beta11/s27_admin_route_collision_probe.php` BEFORE claiming the slice is done.** A real directory under `public/admin/` is resolved by Apache *before* `public/admin/.htaccess`'s `FallbackResource /admin/index.php`, so a directory whose name matches an `AdminRouter::$validPages` entry **silently shadows that panel page** — the page 301s to the directory and the endpoint answers instead. beta.11 S6.2 shipped exactly this by naming an endpoint `account` while `/admin/account` was the My Account page; the probe catches it in one run and was not consulted. It is also why the asset page is routed `media`, not `assets`. |
