...
| `secure/src/classes/IframeSandbox.php` (the install-wide iframe embed policy: file shape, host matching, the same-origin / own-host guard rails) **and** `secure/management/config/embed-policy.json.example` — the class reads the live `embed-policy.json`, the `.example` is its shipped documentation and the source both setup scripts copy; a build bundles the policy at `<secure>/data/embed-policy.json` (`build.php`) | `ARCHITECTURE.md` §8.1 (CSP `frame-src`), `COMMAND_API.md` (`getIframeSandbox`), `ADMIN_PANEL.md` (Embed security page) |
