...
| Registration — `secure/management/command/register.php`, `qs_auth_attempt_register()` (`AuthManagement.php`), `AdminRouter::attemptRegister()`, `secure/admin/templates/pages/register.php` + `login.php` (what registration takes and returns, and where and for how long the assigned username is shown) | the `register` `help.php` entry, `COMMAND_API.md` (*Authentication* **and** the `account.created` row of *The security trail*), `ARCHITECTURE.md` §3 (*Accounts*), `ADMIN_PANEL.md` §1 — the same contract is stated in all of them |
