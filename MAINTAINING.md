...
| **Any code that writes a value into an inline `<script>`** — the runtime handoff and its encoder `qs_inline_script_json()` (`secure/src/functions/runtimeHandoff.php`), the theme scripts in `Page.php` / `PageManagement.php`, the component preview in `public/p/index.php`, the panel's config blocks (`secure/admin/templates/layout.php`, `pages/preview-config.php`), and `CallTransformer::jsSingleQuoted()` (a call argument). A data value goes through the encoder and a call argument through the literal; a new writer that skips them is one the rule does not reach | `DESIGN_DECISIONS.md` (*A data value written into an inline script cannot end or change the script*), the `contentSecurityPolicy.php` docblock (its DATA rule), `ADMIN_PANEL.md` §9.7 (the page-source paragraph), `ARCHITECTURE.md` §8.1 (the `script-src` row) |
...
| `secure/src/functions/SessionManagement.php` (session boot/establish/destroy/touch, the session ini settings it pins, cookie attributes, save path; the login and registration throttles — `qs_login_throttle_*` / `qs_registration_throttle_*`: what an attempt costs and when it is counted) or `AuthManagement.php` `qs_session_auth` / `validateBearerToken` / the generation counter / the login gate `qs_auth_attempt_login` / the password cost and its helpers (`QS_PASSWORD_BCRYPT_COST`, `qs_password_*`) | `ARCHITECTURE.md` §3 + §7 (auth rows), `COMMAND_API.md` (*Authentication*), `ADMIN_PANEL.md` §1 + §6, and the `login` and `register` `help.php` entries, whose notes state the throttles |
| The sign-in doors — `secure/management/command/login.php`, `AdminRouter::attemptLogin()` / `clearToken()` / `attemptSetup()`, `secure/admin/templates/pages/login.php` + `setup.php` (what a sign-in takes and returns, what the first-run form asks for, and what each door writes to the security trail — the panel's must stay the commands' events and payloads) | the `login` `help.php` entry, `COMMAND_API.md` (*Authentication* **and** *The security trail*), `ADMIN_PANEL.md` §1, `ARCHITECTURE.md` §3 |
